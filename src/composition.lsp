...
(defun composition (s1 s2) 
	(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: [ ~S ] [ ~S ]" s1 s2))
	(prog (s1_editable s2_editable s2_element add_element s3 result) 
		(cond
			(; s1 and s2 are nill -> nil + nil = nil
				(and (eq s1 NIL) (eq s2 NIL))
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: composition result NIL"))
				(return-from composition NIL )
			)

			(; s1 is not nil and s2 is nil -> a1 + nil = s1
				(and (not (eq s1 NIL)) (eq s2 NIL) )
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: composition result is s1 because s2 is NIL [ ~S ]" s1))
				(return-from composition s1 )
			)

			(; a1 is nil and s2 is not nil -> nil + s2 = s2
				(and (eq s1 NIL) (not (eq s2 NIL)) )
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: composition result is s2 because s1 is NIL [ ~S ]" s2))
			...
			(; s1 and s2 are not nil 
				...
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: s1 and s2 are not NIL, so applying complex composition"))
				
				; apply s2 sustitution over s1
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: applying s2 sustitution on s1"))
				(setf s3 (multiple_sustitution s1_editable s2_editable))
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: s2 sustitution on s1 result [ ~S ]" s3))
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: joining s3 and s3 into result"))

				; put in s3 the s2 elements which denominator is not in s1 denominators
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: adding s2 elements (which denominator is not in s1 denominators) to s3"))
				(dolist (s2_element s2_editable) 
					(setf add_element (is_element_allowed s2_element s1))
					(if add_element 
						(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: adding s2 element into s3 [ ~S ] " s2_element))
					;else
						(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: not adding s2 element into s3 [ ~S ] " s2_element))
					)
					(when add_element 
						(setf s3 (append s3 (list s2_element)))
					)
				)
				(when (eq debug T) (format t "~%       DEBUG:composition.lsp:composition: the resulting s3 is [ ~S ]" s3))
...
(defun is_element_allowed (sustitution sustitutions_list)
	(prog (sustitution_last sustitution_first sustitution_list_element_last sustitution_list_element_first)
		...
		( dolist (sustitution_list_element sustitutions_list)
			...
			;    return NIL
			(when (eq debug T) (format t "~% ( ~S ~S )  ( ~S ~S ) " sustitution_first sustitution_last sustitution_list_element_first sustitution_list_element_last))
