...
(defun unification_with_atom (e1 e2)
	(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: [ ~S ] [ ~S ]" e1 e2)
	(prog (is_in result)
		(cond
			(; if e2 is atom and e1 not, switch them
				(and (not(is_atom e1)) (is_atom e2) )
				
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e2 is atom and e1 not -> (unification_with_atom e2 e1)")
			...
			(; if e1 == e2 return NIL
				...
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e1 [ ~S ] is equal to e2 [ ~S ] -> return NIL " e1 e2)
			...
			(; if e1 is variable
				...
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e1 is var -> checking if e1 is in e2")
				(when (and (listp e2) (member e1 e2))
					(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e1 is var -> e1 is in e2 -> return unification_error")
					(return-from unification_with_atom 'UNIFICATION_ERROR)
				)
				; making e2/e1
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e1 is var -> e1 is not in e2 -> e2 [ ~S ] / e1 [ ~S ] " e2 e1)
			...
			(; if e2 is variable
				...
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e2 is var -> e1 [ ~S ] / e2 [ ~S ]" e1 e2)
			...
			( T
				...
				(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_atom: e1 and e2 are not var -> return unification_error ")
				(return-from unification_with_atom 'UNIFICATION_ERROR)
			)
		)
	)
...
(defun unification_with_list (e1 e2)
	(logging "debug" NIL t "DEBUG:unification.lsp:unification_with_list: [ ~S ] [ ~S ]" e1 e2)
