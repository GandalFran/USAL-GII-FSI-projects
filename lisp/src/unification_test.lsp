...
(defun test_unification (test_name first_var second_var expected) 
	(when (string= loglevel "debug") (format t "~%TEST:unification_test.lsp: ~S :(unification ~S ~S) (expected: ~S)" test_name first_var second_var expected))
	(setf result (unification first_var second_var))
	(format t "~%TEST:unification_test.lsp: ~S : (unification ~S ~S) = ~S (expected: ~S)" test_name first_var second_var result expected)
)
